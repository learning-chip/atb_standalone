...
#endif
// Standalone build targets Ascend 910B (arch32). OPP may omit arch20 sources; do not fall back to arch20.
    ...
    __gm__ uint8_t *workspace, __gm__ uint8_t *tiling)
{
    ...
    // Standalone <<<>>> launch does not preload g_tilingKey (CANN graph does). Set explicitly so
    // runtime dispatch matches the compiled template (see ifa_standalone_preprocess.h).
